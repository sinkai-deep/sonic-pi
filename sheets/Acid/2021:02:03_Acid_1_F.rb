    ...
    4.times do
      ...
      with_fx :distortion, amp: 0.5, distort: 0.3 do
        with_fx :eq, high: 1.5, high_shelf: 0 do
          if spread(13,16).rotate(num2).tick(:fm1)
            synth :fm,
              note: notes4 + choose([12,24]),
              depth: 1,
              divisor: 2,
      ...
      if spread(25, 32).rotate(7).tick(:tb1)
        synth :tb303,
          note: notes3 -12,
          res: 0.9,
      ...
      with_fx :eq, high: 1, low_shelf: 1, high_shelf: 0 do
        if spread(27, 32).rotate(num).tick(:tb1)
          synth :tb303,
            note: notes-12,
            res: (range 0, 0.9, 0.05).mirror.look(:tb1),
...
with_fx :reverb, room: 0.1 do
  live_loop :pc, sync: :metro do
    ...
    with_fx :eq, amp: 3,low: -2, high: -0.2 do
      sample :drum_cymbal_soft,amp: 1, sustain: 0.1, release: 0.2 if spread(1,4).rotate(2).look
    ...
    ##|   sample :drum_cymbal_soft,amp: 0.3, sustain: 0.1, release: 0.1 if spread(1,4).rotate(2)
    ##| end
    sleep 0.25
  end
end
